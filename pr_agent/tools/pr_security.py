...
class PRSecurityReview:
    def __init__(self, pr_url: str, args: list | None = None,
        ...
        self.vars = {
            ...
            "diff": "",
            "extra_instructions": get_settings().get("pr_security", {}).get(
                "extra_instructions", ""),
        }

        prompt_path = '.ai/pr-agent/prompt/SECURITY.md'
        branch = get_settings().get("PR_HELP_DOCS.REPO_DEFAULT_BRANCH", "main")

        self.user_prompt_template = (
            "You are a security reviewer for a pull request.\n"
            "Diff:\n{{ diff }}\n\n"
            "Provide a bullet list of security issues and suggestions.\n"
            "Answer in Russian.\n"
        )

        try:
            prompt = self.git_provider.get_pr_file_content(prompt_path, branch)
            if prompt:
                self.user_prompt_template = prompt
        except Exception as e:
            get_logger().debug("GET PROMPT", ERROR=e)
        ...
        self.progress = "## Checking security\n\n"
        self.progress += ("\nWork in progress ...<br>\n"
    ...
    async def run(self):
        try:
            if not self.git_provider.get_files():
                get_logger().info(
                    f"PR has no files: {self.pr_url}, skipping security review")
                return None

            get_logger().info("Checking security for PR...")
            if (
                get_settings().config.publish_output and
                get_settings().config.publish_output_progress and
                ...
                if self.git_provider.is_supported("gfm_markdown"):
                    ...
                    self.git_provider.publish_comment(
                        "Preparing security review...", is_temporary=True)
            ...
            if self.prediction and get_settings().config.publish_output:
                if self.progress_response:
                    self.git_provider.edit_comment(
                        self.progress_response, body=self.prediction)
                else:
                    self.git_provider.remove_initial_comment()
                    self.git_provider.publish_comment(self.prediction)
        except Exception as e:
            get_logger().error(f"Error generating security review: {e}")
            if self.progress_response:
    ...
    async def _prepare_prediction(self, model: str):
        ...
        get_logger().debug("GET Prediction", user_prompt=user_prompt)
